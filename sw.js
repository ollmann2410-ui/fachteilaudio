...
self.addEventListener('activate',event=>{
  event.waitUntil((async()=>{
    ...
    await Promise.all(
      names
        .filter(name=>name.startsWith('tf-shell-') && name!==SHELL_CACHE)
...
function rangeResponse(request,response){
  ...
  return response.arrayBuffer().then(buf=>{
    ...
    headers.set('Content-Range',`bytes ${start}-${e}/${size}`);
    headers.set('Accept-Ranges','bytes');
    headers.set('Content-Length',String(e-start+1));
    return new Response(buf.slice(start,e+1),{status:206,statusText:'Partial Content',headers});
  });
}
...
self.addEventListener('fetch',event=>{
  ...
  if(url.pathname.includes('/audio/')){
    event.respondWith((async()=>{
      ...
      if(cached)return rangeResponse(req,cached.clone());
      return fetch(req);
    })());
    return;
  }
